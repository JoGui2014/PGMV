...
public class XMLReader : MonoBehaviour {
    ...
    private Dictionary<Vector2, List<XmlNode>> unitPositions = new Dictionary<Vector2, List<XmlNode>>();
    ...
    void CheckSpecialConditions() {
        foreach (var position in unitPositions) {
            List<XmlNode> units = position.Value;
            bool hasAttack = false;
            bool hasHold = false;
            print(units);

            foreach (XmlNode unit in units) {
                string action = unit.Attributes["action"].Value;
                if (action == "attack") {
                    hasAttack = true;
                } else if (action == "hold") {
                    hasHold = true;
            ...
            if (units.Count >= 2 && hasAttack && hasHold) {
                changeSceneButton.gameObject.SetActive(true);
                print("hello");
                Debug.Log("Special condition met: Two soldiers at the same position, one attacking and one holding.");
                return;
            }
        }
    }
   ...
   void summonPieces(XmlNode unit) {
    ...
    private IEnumerator playLoop() {
        ...
        while (!lastTurn && isRunning) {
            ...
            yield return new WaitForSeconds(3); // Adjust if needed
        }
    }
